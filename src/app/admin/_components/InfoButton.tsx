...
export function InfoButton({ title, body }: Props) {
  ...
  return (
    <div className="relative inline-flex items-center" ref={ref}>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-5 h-5 rounded-full bg-black/6 hover:bg-[#E87722]/15 flex items-center justify-center transition-colors"
      ...
      {open && (
        <div className="absolute left-0 top-7 z-50 w-80 bg-white rounded-2xl border border-black/10 shadow-2xl p-4">
          <div className="flex items-start justify-between gap-2 mb-2">
            <p style={{ ...SYNE, fontWeight: 700, fontSize: 13, color: '#111' }}>{title}</p>
            <button
