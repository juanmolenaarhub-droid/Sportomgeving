...
// ─── Avatar helper ─────────────────────────────────────────────────────────────
function BuddyAvatar({ buddy, size, border }: { buddy: Buddy; size: number; border?: string }) {
  ...
  if (buddy.avatarUrl) {
    return (
      ...
      <img
        src={buddy.avatarUrl}
        alt={buddy.name}
        style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: border ?? 'none' }}
      />
    )
  }
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: border ?? 'none' }}>
      <span style={{ ...SYNE, fontSize: size * 0.33, fontWeight: 900, color: '#fff' }}>{inits}</span>
    </div>
  )
}
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <BuddyAvatar buddy={buddy} size={44} />
          <div style={{ flex: 1 }}>
            <div style={{ ...SYNE, fontWeight: 900, fontSize: 17, color: INK }}>Buddy worden met {buddy.name.split(' ')[0]}</div>
            {buddy.sports[0] && <div style={{ ...DM, fontSize: 12, color: '#888', marginTop: 1 }}>{buddy.sports[0].label}</div>}
          </div>
...
function SearchRow({ buddy, onRequest }: { buddy: Buddy; onRequest: () => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 20px' }}>
      <BuddyAvatar buddy={buddy} size={48} />
...
function FeaturedCard({ buddy, rank, onRequest }: { buddy: Buddy; rank: number; onRequest: () => void }) {
  ...
  return (
    <div style={{ borderRadius: 20, overflow: 'hidden', position: 'relative', background: bg, margin: '0 16px' }}>
      {/* Ornament: groot initialen of foto */}
      {buddy.avatarUrl ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img
          src={buddy.avatarUrl}
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{ padding: '20px 16px 0', display: 'flex', alignItems: 'center', gap: 14 }}>
          <BuddyAvatar buddy={buddy} size={64} border="3px solid rgba(255,255,255,0.5)" />
          <div>
            <div style={{ ...SYNE, fontSize: 28, fontWeight: 900, color: '#fff', letterSpacing: -0.5, lineHeight: 1.1 }}>{buddy.name}</div>
            <div style={{ ...DM, fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.7)', marginTop: 2 }}>
        ...
        <div style={{ padding: '12px 16px 0' }}>
          {buddy.bio && (
            <div style={{ ...DM, fontSize: 13, color: 'rgba(255,255,255,0.8)', fontStyle: 'italic', lineHeight: 1.4, marginBottom: 12 }}>
...
function MiniCard({ buddy, onRequest }: { buddy: Buddy; onRequest: () => void }) {
  ...
  return (
    <div
      style={{ borderRadius: 16, overflow: 'hidden', background: bg, position: 'relative', aspectRatio: '1 / 1.15', cursor: 'pointer' }}
      onClick={onRequest}
    >
      {/* Foto of initialen ornament */}
      {buddy.avatarUrl ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img
          src={buddy.avatarUrl}
      ...
      {/* Naam + info onderaan */}
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '32px 12px 12px', background: 'linear-gradient(to top, rgba(0,0,0,0.65), transparent)' }}>
...
export default function FindPage() {
  ...
  // Stable shuffled "Nieuw" list — computed once when buddies loads
  ...
  useEffect(() => {
    async function load() {
      ...
      const list: Buddy[] = profiles.filter(p => !blockedIds.has(p.id)).map(p => {
        // user_sports entries first; fall back to profile.sport column (e.g. seed accounts)
        let sports = sportsMap[p.id] ?? []
        const profileSport = (p as unknown as Record<string, unknown>).sport as string | null
        if (sports.length === 0 && profileSport) {
          sports = [{ label: profileSport, level: '' }]
        }
      ...
      setBuddies(list)
      // Stable shuffle — computed once
      if (!byNewRef.current) {
        byNewRef.current = [...list].sort(() => Math.random() - 0.5)
      }
      setLoading(false)
    }
    load()
      ...
      try {
        ...
        if (data) {
          ...
          const results: Buddy[] = data.map(p => {
            ...
            // For users not in cache: use profile.sport as fallback
            const profileSport = (p as unknown as Record<string, unknown>).sport as string | null
            return {
              id: p.id,
              name: p.full_name ?? p.username ?? 'Onbekend',
  ...
  function confirmRequest(id: string) {
    ...
    if (byNewRef.current) {
      byNewRef.current = byNewRef.current.map(b => b.id === id ? { ...b, requested: true } : b)
    }
                  ...
                  <div style={{ padding: '24px 16px 0' }}>
                    ...
                    {rest.length > 8 && (
                      <button
                        ...
                        style={{
                          width: '100%', marginTop: 12, height: 48, borderRadius: 14,
                          border: '1.5px solid #e5e7eb', background: 'none',
                          ...SYNE, fontSize: 14, fontWeight: 800, color: INK,
