    ...
    <div className={`relative ${className}`} ref={containerRef}>
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {selected.map(id => {
            ...
            return (
              <span
                key={id}
                className="inline-flex items-center gap-1 bg-[#1E2B20] text-white text-xs px-2.5 py-1 rounded-full"
              >
      ...
      <button
        ...
        className={`w-full flex items-center justify-between gap-2 px-3 py-2.5 text-sm border rounded-lg bg-white transition-all text-left ${
          open
            ? 'border-[#C4F542] ring-2 ring-[#C4F542]/20'
            : error
            ? 'border-red-400'
            : 'border-[#1E2B20]/20 hover:border-[#1E2B20]/40'
      ...
      {open && (
        <div
          ...
          aria-labelledby={label ? labelId : undefined}
          className="absolute left-0 right-0 top-full mt-1 z-50 bg-white border border-[#1E2B20]/10 rounded-xl shadow-lg overflow-hidden"
          style={{ maxHeight: 380 }}
        >
          {/* Search bar */}
          <div className="p-2 border-b border-[#1E2B20]/8">
          ...
          <div ref={listRef} className="overflow-y-auto" style={{ maxHeight: 256 }}>
            {filteredSports ? (
              ...
              <>
                {/* Popular chips */}
                <div>
                  <p className="text-xs font-semibold text-[#1E2B20]/40 uppercase tracking-wider px-3 py-1.5 bg-gray-50">
                    Populair
                  </p>
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border transition-all ${
                            isSelected
                              ...
                              : 'bg-white text-gray-600 border-gray-200 hover:border-[#C4F542] hover:text-[#C4F542]'
                          }`}
                        >
          ...
          {/* ── Anders / eigen sport ─────────────────────────────────────── */}
          <div className="px-3 py-2.5 border-t border-[#1E2B20]/8 bg-gray-50">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-1.5 text-sm bg-white border border-gray-200 rounded-lg outline-none focus:border-[#C4F542] focus:ring-1 focus:ring-[#C4F542]/20 placeholder:text-gray-400 transition-all"
              />
              <button
                type="button"
                onClick={handleCustomSubmit}
                disabled={!customInput.trim()}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#1E2B20] text-white text-xs font-semibold disabled:opacity-30 hover:bg-[#333] transition-colors"
              >
          ...
          {multiple && maxSelections && (
            <div className="px-3 py-2 border-t border-[#1E2B20]/8 bg-white">
              <p className="text-xs text-gray-400">
                {selected.length} / {maxSelections} geselecteerd
                {maxReached && <span className="ml-1 text-[#C4F542] font-semibold">— maximum bereikt</span>}
              </p>
            </div>
          )}
        </div>
...
function SportRow({ label, selected, focused, disabled, multiple, onClick }: SportRowProps) {
  return (
    ...
    >
      {multiple && (
        <span
          className={`w-4 h-4 rounded flex items-center justify-center shrink-0 border-2 transition-colors ${
            selected ? 'bg-[#C4F542] border-[#C4F542]' : 'border-gray-300'
          }`}
        >
      ...
      {!multiple && selected && (
        <Check className="w-4 h-4 text-[#C4F542] shrink-0" />
      )}
    </div>
  )
}
